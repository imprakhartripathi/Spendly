...
import { X, Save, Calendar, DollarSign, Tag, FileText, Repeat } from "lucide-react";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const endpoint = isAutopay 
        ? (transaction ? `/user/${userId}/autopay?transactionId=${transaction._id}` : `/user/${userId}/autopay`)
        : (transaction ? `/user/${userId}/edit/transection?transectionId=${transaction._id}` : `/user/${userId}/add/transection`);
      
      const method = transaction ? 'patch' : 'post';

      await axios[method](`${backendURL}${endpoint}`, formData, {
    ...
    <Portal>
      ...
      >
        <div className="form-header">
          <h2>
            {transaction ? 'Edit' : 'Add'} {isAutopay ? 'Autopay' : 'Transaction'}
          </h2>
